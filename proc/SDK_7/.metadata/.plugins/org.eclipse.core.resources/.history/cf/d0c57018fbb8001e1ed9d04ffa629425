...
//						(u8*)(&TX_Com_buf_pointer->COMMAND.COM_RD_FILE_DATA_buf.DATA[0])
		#if NAND_USE
						status_page = NAND_read_arc_page( (u8*)(&TX_Com_buf_pointer->COMMAND.COM_RD_FILE_DATA_buf.DATA[0]), read_page );
		#else
						////фиктивная функция формирования страницы NAND
//						//status_page +=1;
//						kk = 0;
//						for(ii=0; ii<MAX_LENGTH_ARC_PAGE_def;ii++)
//						{
//							kk++;
//							TX_Com_buf_pointer->COMMAND.COM_RD_FILE_DATA_buf.DATA[ii]=ii;
//						}
//						//status_page ^= 1;
//						status_page = 0;
////конец фиктивной функции формирования страницы NAND
		#endif //NAND_USE
						...
						{
							p_mrte_buff->len = SIZE_ANSWER_COM_RD_ARC_FILE_DATA - MAX_LENGTH_ARC_PAGE_def;
		...
		case COMMAND_GET_INFO:
		{
			...
			{
				answer_length = SIZE_ANSWER_COM_RD_GET_INFO;
				configASSERT((answer_length + Length_CRC) <= MSG_MAX_LEN);
				TX_Com_buf_pointer->COMMAND.COMMAND_GET_INFO_buf.ModuleType = device_type;
				TX_Com_buf_pointer->COMMAND.COMMAND_GET_INFO_buf.TimerTick = GetTimerTick();
				TX_Com_buf_pointer->COMMAND.COMMAND_GET_INFO_buf.TimeUTC = GetUTCTime();
				TX_Com_buf_pointer->COMMAND.COMMAND_GET_INFO_buf.ModuleSN = get_serial_num();
				//
				for(int ii=0; ii<LENGTH_VER_PO; ii++)
				{
					((u8*)(&TX_Com_buf_pointer->COMMAND.COMMAND_GET_INFO_buf.VER_PO))[ii] = ver_pro[ii];
				}
				TX_Com_buf_pointer->COMMAND.COMMAND_GET_INFO_buf.COUNTER_INCLUSION = get_inclusion_counter();
				//
				copy_time_worked(&TX_Com_buf_pointer->COMMAND.COMMAND_GET_INFO_buf.Time_Worked);
				//
				{
					SET_TIMER_RTX_type timer_tmp;
					copy_msmt_timer(&timer_tmp);
					TX_Com_buf_pointer->COMMAND.COMMAND_GET_INFO_buf.Alarm_Time 					= timer_tmp.start_time;
					TX_Com_buf_pointer->COMMAND.COMMAND_GET_INFO_buf.Time_STOP_Of_Work 				= timer_tmp.stop_time;
					TX_Com_buf_pointer->COMMAND.COMMAND_GET_INFO_buf.COM_Measure_AS_CmdPeriod 		= timer_tmp.base_tlm_rate;
					TX_Com_buf_pointer->COMMAND.COMMAND_GET_INFO_buf.COM_Tesh_Measure_AS_CmdPeriod 	= timer_tmp.ext_tlm_rate;
				}
				TX_Com_buf_pointer->COMMAND.COMMAND_GET_INFO_buf.MAX_Length_Package = MAX_LENGTH_PACKAGE_def;
				TX_Com_buf_pointer->COMMAND.COMMAND_GET_INFO_buf.Length_File_LOG = (TASK_LOG_FILE_SIZE);//TODO: Make it 2kB equally? now it is 4*528 Bytes
				TX_Com_buf_pointer->COMMAND.COMMAND_GET_INFO_buf.Length_File_CAL = (CALIB_FILE_SIZE); //TODO: May be change to sizeof(calib_file_type)
				TX_Com_buf_pointer->COMMAND.COMMAND_GET_INFO_buf.Length_File_ARC[0] = nand_get_flash_size();
				TX_Com_buf_pointer->COMMAND.COMMAND_GET_INFO_buf.Length_File_ARC[1] = 0;
				TX_Com_buf_pointer->COMMAND.COMMAND_GET_INFO_buf.Length_Block_ARC = nand_get_flash_page_size();
				TX_Com_buf_pointer->COMMAND.COMMAND_GET_INFO_buf.Count_Block_ARC = nand_get_flash_pages_num();
				TX_Com_buf_pointer->COMMAND.COMMAND_GET_INFO_buf.Count_Bad_Block_ARC = nand_get_bad_block_count();
				TX_Com_buf_pointer->COMMAND.COMMAND_GET_INFO_buf.Status = GetMsmtStatus();
				{
		...
		{
			u16		FILE_SEL;
			u32		START_ADDRES[2] = {0,0}; 				// Íà÷àëüíûé àäðåñ
			...
			else
			{
				...
				switch(FILE_SEL)
				{
					case FILE_PROG_com:
					{
						...
						if((STOP_ADDRES[0] > (MAX_FIRMWARE_PAGES*PAGE_SIZE)) || (START_ADDRES[0] >= STOP_ADDRES[0]) )
						{
							Prepare_Buf_ERROR(RX_Com_buf_pointer, TX_Com_buf_pointer, ERROR_PARAM_OUT_OF_RANGE, &answer_length);
						}
						else
						{
							//
							page_addr = START_ADDRES[0] / PAGE_SIZE + FIRMWARE_NORMAL_PAGE;
							wr_length = STOP_ADDRES[0] - START_ADDRES[0];
					...
					case FILE_LOG_com:
					{
						if((STOP_ADDRES[0] > (TASK_LOG_FILE_SIZE)) || (START_ADDRES[0] >= STOP_ADDRES[0]))
						{
							Prepare_Buf_ERROR(RX_Com_buf_pointer, TX_Com_buf_pointer, ERROR_PARAM_OUT_OF_RANGE, &answer_length);
						}
						else
						{
							page_addr = START_ADDRES[0] / PAGE_SIZE + TASK_LOG_BEGIN_PAGE;
							page_offset = START_ADDRES[0] % PAGE_SIZE;
							wr_length = STOP_ADDRES[0] - START_ADDRES[0];
							ERR = 0;
							//
							//Start new note in NAND archive
		#if NAND_USE
							if(START_ADDRES[0] == 0)
							{
								NAND_new_note();
					...
					case FILE_CAL_com:
					{
						if((STOP_ADDRES[0] > (CALIB_FILE_SIZE)) || (START_ADDRES[0] >= STOP_ADDRES[0]))
						{
							Prepare_Buf_ERROR(RX_Com_buf_pointer, TX_Com_buf_pointer, ERROR_PARAM_OUT_OF_RANGE, &answer_length);
						}
						else
						{
							page_addr = START_ADDRES[0] / PAGE_SIZE + CALIB_FILE_BEGIN_PAGE;
							page_offset = START_ADDRES[0] % PAGE_SIZE;
							wr_length = STOP_ADDRES[0] - START_ADDRES[0];
							ERR = 0;
							//
				...
				if(ERR == 0)
				{
					if( xSemaphoreTake( spi_rw_mutex, 250  ) == pdTRUE )
					{
						u8  *p_buff_temp = (u8*)SpiFFBuffGetPtr();
						u8	*p_src_temp = (u8*)&(RX_Com_buf_pointer->COMMAND.COM_WR_FILE_DATA_buf.DATA[0]);
						//
						...
						//
						if((page_offset == 0) && ((wr_length % PAGE_SIZE) == 0))
						{
							if(write_spi_flash(page_addr, p_src_temp, wr_length) != XST_SUCCESS)
							{
								Prepare_Buf_ERROR(RX_Com_buf_pointer, TX_Com_buf_pointer, ERROR_HARDWARE, &answer_length);
								inc_spi_err_cnt();
								xSemaphoreGive(spi_rw_mutex);
							...
							wr_length = 0;//Succesfully writed data
						}
						else
						{
							...
							do
							{
								u32 req_len = wr_length;
								if(req_len > (PAGE_SIZE - page_offset))
								{
									req_len = (PAGE_SIZE - page_offset);
								}
								if( (page_offset != 0) || (((req_len+page_offset) % PAGE_SIZE) != 0) )
								{
									if(read_spi_flash( (page_addr*PAGE_SIZE), p_buff_temp, PAGE_SIZE ) != XST_SUCCESS)
									{
										Prepare_Buf_ERROR(RX_Com_buf_pointer, TX_Com_buf_pointer, ERROR_HARDWARE, &answer_length);
										inc_spi_err_cnt();
										break;
									}
									memcpy(&p_buff_temp[page_offset], p_src_temp, req_len);
									if(write_spi_flash(page_addr, p_buff_temp, (req_len+page_offset)) != XST_SUCCESS)
									{
										Prepare_Buf_ERROR(RX_Com_buf_pointer, TX_Com_buf_pointer, ERROR_HARDWARE, &answer_length);
										inc_spi_err_cnt();
										break;
								...
								{
									if(write_spi_flash(page_addr, p_src_temp, req_len) != XST_SUCCESS)
									{
										Prepare_Buf_ERROR(RX_Com_buf_pointer, TX_Com_buf_pointer, ERROR_HARDWARE, &answer_length);
										inc_spi_err_cnt();
										break;
						...
						if(wr_length == 0)
						{
							TX_Com_buf_pointer->COMMAND.COM_WR_FILE_DATA_buf.FILE_SEL 		 = FILE_SEL;
							TX_Com_buf_pointer->COMMAND.COM_WR_FILE_DATA_buf.START_ADDRES[0] = START_ADDRES[0];
							TX_Com_buf_pointer->COMMAND.COM_WR_FILE_DATA_buf.START_ADDRES[1] = 0;
							TX_Com_buf_pointer->COMMAND.COM_WR_FILE_DATA_buf.STOP_ADDRES[0]  = STOP_ADDRES[0];
							TX_Com_buf_pointer->COMMAND.COM_WR_FILE_DATA_buf.STOP_ADDRES[1]  = 0;
							answer_length = SIZE_ANSWER_COM_WR_FILE_DATA;
						}
					}
					else
		...
		case COMMAND_READ_FILE_DATA:
		{
			...
			else
			{
				...
				switch(FILE_SEL)
				{
					...
					case FILE_PROG_com:
					{
						if((START_ADDRES[0] >= (MAX_FIRMWARE_PAGES*PAGE_SIZE)) || (START_ADDRES[0] >= STOP_ADDRES[0]))
						{
							Prepare_Buf_ERROR(RX_Com_buf_pointer, TX_Com_buf_pointer, ERROR_PARAMS_CONFLICT, &answer_length);
						}
						else
						{
							//						page_addr = START_ADDRES[0] / PAGE_SIZE + FIRMWARE_NORMAL_PAGE;
							//						page_offset = START_ADDRES[0] % PAGE_SIZE;
							rd_length = (STOP_ADDRES[0] > (MAX_FIRMWARE_PAGES*PAGE_SIZE)) ? ((MAX_FIRMWARE_PAGES*PAGE_SIZE) - START_ADDRES[0]) : (STOP_ADDRES[0] - START_ADDRES[0]);
							read_addr = START_ADDRES[0] + (FIRMWARE_NORMAL_PAGE * PAGE_SIZE);
...
//						else
//						{
//							//						page_addr = START_ADDRES[0] / PAGE_SIZE + FIRMWARE_GOLDEN_PAGE;
//							//						page_offset = START_ADDRES[0] % PAGE_SIZE;
//							rd_length = (STOP_ADDRES[0] > (MAX_FIRMWARE_PAGES*PAGE_SIZE)) ? ((MAX_FIRMWARE_PAGES*PAGE_SIZE) - START_ADDRES[0]) : (STOP_ADDRES[0] - START_ADDRES[0]);
//							read_addr = START_ADDRES[0] + (FIRMWARE_GOLDEN_PAGE * PAGE_SIZE);
//							//						page_num = rd_length / PAGE_SIZE + (((rd_length % PAGE_SIZE) > 0) ? 1 : 0);
//
//							ERR = 0;
//
...
//					}
					case FILE_LOG_com:
					{
						if((START_ADDRES[0] >= (TASK_LOG_FILE_SIZE)) || (START_ADDRES[0] >= STOP_ADDRES[0]))
						{
							Prepare_Buf_ERROR(RX_Com_buf_pointer, TX_Com_buf_pointer, ERROR_PARAM_OUT_OF_RANGE, &answer_length);
						}
						else
						{
							//						page_addr = START_ADDRES[0] / PAGE_SIZE + TASK_LOG_BEGIN_PAGE;
							//						page_offset = START_ADDRES[0] % PAGE_SIZE;
							rd_length = (STOP_ADDRES[0] > TASK_LOG_FILE_SIZE) ? (TASK_LOG_FILE_SIZE - START_ADDRES[0]) : (STOP_ADDRES[0] - START_ADDRES[0]);
							read_addr = START_ADDRES[0] + (TASK_LOG_BEGIN_PAGE * PAGE_SIZE);
							//						page_num = rd_length / PAGE_SIZE + (((rd_length % PAGE_SIZE) > 0) ? 1 : 0);
							ERR = 0;
						}
						break;
					...
					case FILE_CAL_com:
					{
						if((START_ADDRES[0] >= (CALIB_FILE_SIZE)) || (START_ADDRES[0] >= STOP_ADDRES[0]))//TODO: May be change to sizeof(calib_file_type)
						{
							Prepare_Buf_ERROR(RX_Com_buf_pointer, TX_Com_buf_pointer, ERROR_PARAM_OUT_OF_RANGE, &answer_length);
						}
						else
						{
							//						page_addr = START_ADDRES[0] / PAGE_SIZE + CALIB_FILE_BEGIN_PAGE;
							//						page_offset = START_ADDRES[0] % PAGE_SIZE;
							rd_length = (STOP_ADDRES[0] > CALIB_FILE_SIZE) ? (CALIB_FILE_SIZE - START_ADDRES[0]) : (STOP_ADDRES[0] - START_ADDRES[0]);
							read_addr = START_ADDRES[0] + (CALIB_FILE_BEGIN_PAGE * PAGE_SIZE);
							//						page_num = rd_length / PAGE_SIZE + (((rd_length % PAGE_SIZE) > 0) ? 1 : 0);
							ERR = 0;
							//
						}
			...
			else
			{
				...
				//if (1)
				{
					...
					//set_led(LED_0);
					{
						...
						if ( copy_flash(FIRMWARE_NORMAL_PAGE, FIRMWARE_GOLDEN_PAGE, MAX_FIRMWARE_PAGES * PAGE_SIZE, 1) != XST_SUCCESS )
						{
							set_flash_protection(1);
							Prepare_Buf_ERROR(RX_Com_buf_pointer, TX_Com_buf_pointer, ERROR_PARAMS_SIZE, &answer_length);
							break;
...
//		case COMMAND_RESET_WORK_INFO:
//		{
//			if(request_length != SIZE_RECEIVE_COMMAND_CLEAR_WORK_INFO)
//			{
//				Prepare_Buf_ERROR(RX_Com_buf_pointer, TX_Com_buf_pointer, ERROR_PARAMS_SIZE, &answer_length);
//			}
//			else
//			{
//				//Compare Sequre Key
//				if(RX_Com_buf_pointer->COMMAND.CLEAR_WORK_INFO_MAGIC_word.sequrity_key == CLEAR_WORK_INFO_MAGIC_KEY)
//				{
//				//Clear Work Info
//					if(reset_time_worked() == XST_SUCCESS)
//					{
//						answer_length = SIZE_ANSWER_COMMAND_CLEAR_WORK_INFO;
//						configASSERT((answer_length + Length_CRC) <= MSG_MAX_LEN);
//						TX_Com_buf_pointer->COMMAND.CLEAR_WORK_INFO_MAGIC_word.sequrity_key = 0;
//					}
//					else
//					{
//						Prepare_Buf_ERROR(RX_Com_buf_pointer, TX_Com_buf_pointer, ERROR_HARDWARE, &answer_length);
...
//		}
		...
		case COMMAND_SET_MODE_SP:
		{
			...
			else
			{
				...
				TX_Com_buf_pointer->COMMAND.SET_MODE_SP.mode_sp = u8GetModeSp();  // вернули записанное значение
...
//		case COMMAND_:
//		{
//			if(request_length != SIZE_RECEIVE_COM_)
//			{
//				Prepare_Buf_ERROR(RX_Com_buf_ponter, TX_Com_buf_ponter, ERROR_PARAMS_SIZE, &answer_length);
//			}
//			else
//			{
//				//
//				answer_length = SIZE_ANSWER_COM_;
//				configASSERT((answer_length + Length_CRC) <= MSG_MAX_LEN);
//				//
//			}
//			break;
