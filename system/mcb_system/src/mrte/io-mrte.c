...
void can_rx_mrte_task(void *pvParameters)
{
	...
	{
//		set_led(LED_0);
		if(can_recieve_msg( &can_message, ( portTickType ) portMAX_DELAY ) == pdTRUE)
				{
					//set_led(LED_1);
					//GPIO_SetBit(GPIO_LED_2); // TODO: GPIO_LED_2
					if(is_mrte_reset(can_message.arbcode))		// CAN_ID == 0
					{
						// Reset process with appropriate answer
						// is carried out in function is_mrte_reset()
					}
					...
					else if(on_can_recv_message_hook(&can_message))
					{
						mrte_push(&can_message);
					}
					//clr_led(LED_1);
					//GPIO_ResetBit(GPIO_LED_2);// TODO: GPIO_LED_2
				}
	}
}
